  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        setError(null);
        console.log(
          "Fetching instant meal products from Shopify collection..."
        );
        ...
        if (!shopifyProducts || shopifyProducts.length === 0) {
          console.log("No products found in instant-meals collection");
          setProducts([]);
          return;
        }
        ...
        console.log(`Loaded ${limitedProducts.length} instant meal products`);
      } catch (err: any) {
        console.error("Error fetching instant meal products:", err);
        setError(`Failed to load instant meal products: ${err.message}`);
        setProducts([]);
      } finally {
        setLoading(false);
      }
      ...
      <div style={{ marginLeft: "180.4px", marginRight: "180.4px" }}>
          ...
          <div className={styles.heroInner}>
            <div className={styles.textWrapper}>
              ...
              <div className={styles.description}>
                <p>
                  <strong>
                    Quick, nutritious, and delicious instant meals for your busy
                    lifestyle.
                  </strong>
                </p>
                <p>
                  Discover our range of healthy instant meals that don't
                  compromise on taste or nutrition. Perfect for busy
                  professionals, students, or anyone looking for a quick and
                  wholesome meal solution. Each meal is carefully crafted with
                  premium ingredients to fuel your day.
                </p>
              </div>
            </div>
          </div>
        ...
        {!loading && !error && products.length === 0 && (
          <div className={listStyles.noProducts}>
            No instant meal products found in the collection.
          </div>
        )}
