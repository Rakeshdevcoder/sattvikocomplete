  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        setLoading(true);
        setError(null);
        console.log("Fetching combo products from Shopify collection...");
        ...
        if (!shopifyProducts || shopifyProducts.length === 0) {
          console.log("No products found in combos collection");
          setProducts([]);
          return;
        }
        ...
        console.log(`Loaded ${limitedProducts.length} combo products`);
      } catch (err: any) {
        console.error("Error fetching combo products:", err);
        setError(`Failed to load combo products: ${err.message}`);
        setProducts([]);
      } finally {
        setLoading(false);
      }
  ...
  return (
    <>
      <div style={{ marginLeft: "180.4px", marginRight: "180.4px" }}>
        ...
        {/* Product List */}
        {loading && (
          <div className={listStyles.loading}>Loading combo products...</div>
        )}
        ...
        {!loading && !error && products.length === 0 && (
          <div className={listStyles.noProducts}>
            No combo products found in the collection.
          </div>
        )}
